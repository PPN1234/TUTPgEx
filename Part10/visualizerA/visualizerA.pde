...
void setup(){
  ...
  player = minim.loadFile("SpagHeddy.mp3");
  player.loop();
  background(1);
  colorMode(RGB, 255);
  isPaused = true;
...
void visualizer(int r, int posx, int posy) {
  ...
  for (int i = 0; i < bsize; i+=10) {
    float x = (r - 50)*cos(i*2*PI/bsize);
    float y = (r - 50)*sin(i*2*PI/bsize);
    float x2 = (r - 50 + player.left.get(i)*100)*cos(i*2*PI/bsize);
    float y2 = (r - 50 + player.left.get(i)*100)*sin(i*2*PI/bsize);
